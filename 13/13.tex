...
\begin{frame}
  ...
  \begin{block}{Multilevel CC algorithm (MLCC) algorithm}
    ...
    \end{itemize}
  \end{block}
  \begin{yellowblock}{The performance of MLCC algorithm}
...
\begin{frame}
  ...
  \begin{block}{}
    ...
    \begin{itemize}
      ...
      \item Therefore, some learning-based grouping methods come to play.
    \end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{CCEA-AVP algorithm}
    ...
    \end{itemize}
  \end{block}
  \begin{blueblock}{CCEA-AVP with an adaptive grouping method}
    Inspired by CCEA-AVP, CCEA-AVP with an adaptive grouping method was proposed. However, the method based on correlation coefficient has a \alert{large amount of calculations}, and it cannot identify the \alert{nonlinear dependence between variables}.
  \end{blueblock}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{CBCC algorithm}
    A co-evolutionary algorithm was proposed to \alert{allocate computing resources} according to the \alert{contribution of each sub-component} called contribution based cooperative co-evolution(CBCC).
  \end{block}
  \begin{blueblock}{The performance of CBCC algorithm}
    The results show that if there is an \alert{imbalance among the separable and non-separable parts of the fitness value} in the LSGO problem, this method can \alert{significantly reduce the computational time}. 
  \end{blueblock}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Non-decomposition-based Methods}
    \begin{itemize}
      \item The non-decomposed method mainly attempts to improve the performance of the standard meta heuristic algorithm to solve the LSGO problem. 
      \item Such algorithms focus on defining new mutations, selection and crossover operators, hybridization, opposition-based learning, designing and using local search, sampling operators, or variable population size methods.
    \end{itemize}
  \end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
